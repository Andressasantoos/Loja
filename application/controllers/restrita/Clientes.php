    ...
    public function core($cliente_id = null){
        ...
        if(!$cliente_id){
            ...
            //Cadastrar
                  ...
                  $this->form_validation->set_rules('cliente_cpf', 'CPF', 'trim|required|exact_length[14]|is_unique[users.user_cpf]');
                 ...
                 if ($cliente_telefone_fixo) {
                  ...
                  $this->form_validation->set_rules('password', 'Senha', 'trim|min_length[5]|max_length[200]');
             ...
             if(!$cliente = $this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id))){
                 ...
                 if ($cliente_telefone_fixo) {
                  ...
                  if($this->form_validation->run()){
                    ...
                    //Atualiza a senha se a mesma vier no POST
                    if($password){
    ...
    if (!$cliente_id) {
      ...
      public function delete($cliente_id = NULL){
           ...
           if(!$this->ion_auth->is_admin()){
               $this->session->set_flashdata('erro', 'Você não tem permissão para acessar');
               redirect('restrita/clientes');
           }
           ...
           if ($this->core_model->get_by_id('users', array('cliente_user_id' => $cliente_id, 'active' => 1))) {
                 $this->session->set_flashdata('erro', 'Não é possivel excluir um cliente com o usuario ativo');
                 redirect('restrita/clientes');              
           }
